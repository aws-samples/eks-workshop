...
Let's run the same inbuilt example scripts that calculates the value of pi, but this time lets increase the number of executors to 5 by modifying `spark.executor.instances`. 
...
aws emr-containers start-job-run \
  ...
  --job-driver='{
    "sparkSubmitJobDriver": {
      ...
      "sparkSubmitParameters": "--conf spark.executor.instances=5 --conf spark.executor.memory=1G --conf spark.executor.cores=1 --conf spark.driver.cores=1"
    }
  }'\
